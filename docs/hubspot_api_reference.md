...
**Scopes required for read-only sync (Phase 8):**

- `crm.objects.companies.read`
- `crm.objects.deals.read`
- `crm.objects.contacts.read` *(only if we sync contacts later)*
- `crm.schemas.companies.read` *(to list available properties)*
- `crm.schemas.deals.read`

**Storage in our backend:**
- Token stored as env var `HUBSPOT_PRIVATE_APP_TOKEN` (never in DB, never in git).
...
When you share test credentials + a sample call, we confirm:

| # | Question | Why it matters |
|---|---|---|
| 1 | What HubSpot tier is BSG account on? | Determines daily quota (250k / 625k / 1M). |
| 2 | What's the full `properties` object on a typical Company in BSG? | We pick which to mirror to our `companies` table + extract into named columns. |
| 3 | What's the full `properties` object on a typical Deal? | Same. |
| 4 | Is `dealstage` a word-ID (`"contractsent"`) or numeric (`"11348542"`)? | Affects how we display + how we map to our UI labels. |
| 5 | What pipelines exist in BSG (just "default", or multiple)? | We may need a pipeline picker on the wizard. |
| 6 | How many companies + deals total approximately? | Determines whether initial sync needs pagination optimisation. |
| 7 | What's the typical HubSpot company ID shape? Numeric only? Length? | Determines the **last 6 digits** for our `BSG-7100000-XXXXXX` number suffix. |
| 8 | Is there a "Bsg pricing schedule URL" custom property already in HubSpot? | If yes, Phase 9 can write back to it. |
| 9 | What `industry` values are common? Standard HubSpot enum or custom? | UI display in our companies list. |
| 10 | Confirm rate limit error response shape (429 body) | Backend retry logic. |
