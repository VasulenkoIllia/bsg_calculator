...
3. **HubSpot integration is live.** The backend reads companies/deals from HubSpot (TTL-cached), processes inbound webhooks (HMAC v3), and writes document Notes back. Code lives in `server/modules/hubspot/**` and `server/modules/documents/sync.service.ts`. Field selection is in `docs/bsg_hubspot_field_mapping.md`. Never call HubSpot directly from the frontend — all sync goes through backend endpoints.
