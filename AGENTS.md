...
3. **The CRM integration is provider-switched (`CRM_PROVIDER`), and HubSpot is being retired after 2026-08-31.** While `CRM_PROVIDER=hubspot` — the default and shipped state — everything in this rule holds. Under `CRM_PROVIDER=monday` the same flows run against monday.com instead: `server/modules/monday/**` and `server/modules/crm-notes/**`, with `docs/monday_migration_plan.md` as the authoritative reference. Never assume one CRM: read the provider. **HubSpot integration is live.** The backend reads companies/deals from HubSpot (TTL-cached), processes inbound webhooks (HMAC v3), and writes document Notes back. Code lives in `server/modules/hubspot/**` and `server/modules/documents/sync.service.ts`. Field selection is in `docs/bsg_hubspot_field_mapping.md`. Never call HubSpot directly from the frontend — all sync goes through backend endpoints.
