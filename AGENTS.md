...
3. **monday.com is the only CRM; HubSpot is retired.** Production has run `CRM_PROVIDER=monday` since 2026-08-28, and the HubSpot account no longer exists — there is no HubSpot fallback and no rollback to it. The HubSpot code (`server/modules/hubspot/**`, its `/api/v1/hubspot/*` routes, `src/api/hubspot.ts`) is still in the repo but dormant; do not route new work through it. CRM work goes through `server/modules/monday/**` (reads, inbound webhooks, TTL refresh, scheduled backfill) and `server/modules/crm-notes/**` (note write-back and the `crm_notes` ledger), called from the document and calculator `sync.service.ts` files. The authoritative reference is `docs/CRM_INTEGRATION.md`. The code default in `server/config/env.ts` is still `CRM_PROVIDER=hubspot` (tests rely on it), so every deployment must set `CRM_PROVIDER=monday` explicitly; run the server tests as `CRM_PROVIDER=hubspot npm run test:server`, because a `.env` copied from `.env.example` sets `monday` and a shell variable wins over `.env`. Identifiers keep their legacy `hubspot` names by design (`hubspot_company_id`, `hubspot_modified_at`, `hubspotSyncState`, `HUBSPOT_SYNC_TTL_SECONDS`, `AUTO_SYNC_TO_HUBSPOT`, …); do not rename them outside a deliberate, coordinated change. Never call the CRM directly from the frontend — all sync goes through backend endpoints.
4. **Backend is fully implemented and serves the SPA.** `server/` is a complete Express/Drizzle/Postgres API (auth + RBAC, opt-in TOTP 2FA, users/companies/deals/documents/calculator-configs/events/invites/admin-audit, server-side PDF via Puppeteer) and serves the built SPA in production (single container). Extend it with normal backend-task discipline. See `docs/architecture.md`, `docs/backend_conventions.md`, and `docs/ONBOARDING.md`.
