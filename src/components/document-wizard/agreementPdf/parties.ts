...
export function buildPartiesBlock(payload: DocumentTemplatePayload): string {
  ...
  return `<section class="agreement-section">
    <h2 class="agreement-h2">Parties</h2>
    <p class="agreement-p">This Service Agreement (the "Agreement") is entered into between:</p>
    <p class="agreement-p">${escapeHtml(BSG_ENTITY.name)}, a company incorporated under the laws of ${escapeHtml(BSG_ENTITY.jurisdiction)}, having its registered office at ${escapeHtml(BSG_ENTITY.registeredOffice)} ("${escapeHtml(BSG_ENTITY.shortLabel)}"), acting as a technical gateway and merchant onboarding service provider, and</p>
