 ...
 * matches "BSG-7100024"). Sprint 6 may add scope/company filter
 * chips on top.
 */
...
export function DocumentsListPage() {
  ...
  const {
    ...
    fetchNextPage,
    isFetchingNextPage
  } = useDocuments({ q: trimmed.length > 0 ? trimmed : undefined });
      ...
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <label className="flex flex-col gap-1">
          <span className="text-xs font-semibold uppercase tracking-wide text-slate-600">
            Search by number{isBackgroundRefetching ? <span className="ml-2 font-normal lowercase text-slate-400">· refreshing…</span> : null}
          </span>
          <input
            type="search"
            value={search}
